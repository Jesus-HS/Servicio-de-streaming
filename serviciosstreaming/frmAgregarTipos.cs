...
namespace serviciosstreaming
{
    public partial class frmAgregarTipos : Form
    {
        ...
        {
            query.Consulta("CALL insertarTipo('" + txtTexto.Text + "')");
            mostrar();
        }

        private void btnEliminar_Click(object sender, EventArgs e)
        {
            query.Consulta("DELETE FROM tipos WHERE nombreTipo = '" + dgv.CurrentCell.Value + "'");
            mostrar();
        }

        private void btnEditar_Click(object sender, EventArgs e)
        {
            query.Consulta("UPDATE tipos set nombreTipo = '" + txtTexto.Text + "' WHERE nombreTipo ='" + dgv.CurrentCell.Value + "'");
            mostrar();
        }

        private void btnBuscar_Click(object sender, EventArgs e)
        {
            dgv.DataSource = query.obtenerTabla("SELECT nombreTipo FROM tipos WHERE nombreTipo ='" + txtTexto.Text + "'");
        }
