...
namespace serviciosstreaming
{
    public partial class frmAgregarActores : Form
    {
        ...
        {
            query.Consulta("DELETE FROM actores WHERE nombreActor = '" + dgv.CurrentCell.Value + "'");
            mostrarActores();
        }

        private void btnEditarActor_Click(object sender, EventArgs e)
        {
            query.Consulta("UPDATE actores set nombreActor = '" + txtActor.Text + "' WHERE nombreActor ='"+ dgv.CurrentCell.Value + "'");
            mostrarActores();
        }

        private void btnBuscarActor_Click(object sender, EventArgs e)
        {
            dgv.DataSource = query.obtenerTabla("SELECT nombreActor FROM actores WHERE nombreActor ='" + txtActor.Text + "'");
        }
