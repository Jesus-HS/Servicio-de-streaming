...
namespace serviciosstreaming
{
    public partial class frmAgregarGeneros : Form
    {
        ...
        {
            query.Consulta("CALL insertarGeneros('" + txtTexto.Text + "')");
            mostrar();
        }

        private void btnEliminar_Click(object sender, EventArgs e)
        {
            query.Consulta("DELETE FROM generos WHERE nombreGenero = '" + dgv.CurrentCell.Value + "'");
            mostrar();
        }

        private void btnEditar_Click(object sender, EventArgs e)
        {
            query.Consulta("UPDATE generos set nombreGenero = '" + txtTexto.Text + "' WHERE nombreGenero ='" + dgv.CurrentCell.Value + "'");
            mostrar();
        }

        private void btnBuscar_Click(object sender, EventArgs e)
        {
            dgv.DataSource = query.obtenerTabla("SELECT nombreGenero FROM generos WHERE nombreGenero ='" + txtTexto.Text + "'");
        }
