...
namespace serviciosstreaming
{
    public partial class frmAgregarDirectores : Form
    {
        ...
        {
            query.Consulta("CALL insertarDirector('" + txtTexto.Text + "')");
            mostrar();
        }

        private void btnEliminar_Click(object sender, EventArgs e)
        {
            query.Consulta("DELETE FROM directores WHERE nombreDirector = '" + dgv.CurrentCell.Value + "'");
            mostrar();
        }

        private void btnEditar_Click(object sender, EventArgs e)
        {
            query.Consulta("UPDATE directores set nombreDirector = '" + txtTexto.Text + "' WHERE nombreDirector ='" + dgv.CurrentCell.Value + "'");
            mostrar();
        }

        private void btnBuscar_Click(object sender, EventArgs e)
        {
            dgv.DataSource = query.obtenerTabla("SELECT nombreDirector 'Nombre del Director' FROM directores WHERE nombreDirector ='" + txtTexto.Text + "'");
        }
